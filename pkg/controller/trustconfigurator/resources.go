...
func init() {
	...
	utilruntime.Must(trustconfigv1alpha1.AddToScheme(trustConfiguratorScheme))
	...
	versions := schema.GroupVersions([]schema.GroupVersion{trustconfigv1alpha1.SchemeGroupVersion})
	trustConfiguratorCodec = serializer.NewCodecFactory(trustConfiguratorScheme).CodecForVersions(ser, ser, versions, versions)
}

func getSourceResources(oidcConfig trustconfigv1alpha1.OIDCConfig, namespace, genericKubeconfigName, gardenAccessSecretName, serverTLSSecretName string) (map[string][]byte, error) {
	gardenShootTrustConfiguratorConfiguration := trustconfigv1alpha1.GardenShootTrustConfiguratorConfiguration{
		LogLevel:  "info",
		LogFormat: "json",
		Controllers: trustconfigv1alpha1.ControllerConfiguration{
			Shoot: trustconfigv1alpha1.ShootControllerConfig{
				SyncPeriod: &metav1.Duration{Duration: 1 * time.Hour},
				OIDCConfig: &oidcConfig,
			},
			GarbageCollector: trustconfigv1alpha1.GarbageCollectorControllerConfig{
				SyncPeriod:            &metav1.Duration{Duration: 1 * time.Hour},
		...
		LeaderElection: &componentbaseconfigv1alpha1.LeaderElectionConfiguration{
			...
			ResourceLock:      "leases",
			ResourceName:      trustconfigv1alpha1.DefaultLockObjectName,
			ResourceNamespace: trustconfigv1alpha1.DefaultLockObjectNamespace,
		},
		Server: trustconfigv1alpha1.ServerConfiguration{
			HealthProbes: &trustconfigv1alpha1.Server{
				Port: int(healthProbesPort),
			},
			Metrics: &trustconfigv1alpha1.Server{
				Port: int(metricsPort),
			},
			Webhooks: trustconfigv1alpha1.HTTPSServer{
				Server: trustconfigv1alpha1.Server{
					Port: int(webhookPort),
				},
				TLS: trustconfigv1alpha1.TLS{
					ServerCertDir: constants.GardenShootTrustConfiguratorWebhookTLSCertDir,
				},
			},
		},
	}
	...
	configMap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      constants.GardenShootTrustConfiguratorConfigName,
			Namespace: namespace,
			Labels:    getLabels(),
		},
	...
	image, err := imagevector.ImageVector().FindImage(constants.GardenShootTrustConfiguratorImageName)
	if err != nil {
		return nil, fmt.Errorf("failed to find image version for %s: %v", constants.GardenShootTrustConfiguratorImageName, err)
	}
		...
		Spec: appsv1.DeploymentSpec{
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					ServiceAccountName:           constants.GardenShootTrustConfiguratorApplicationName,
					...
					Containers: []corev1.Container{{
						...
						Args: []string{
							...
							"--config=" + fmt.Sprintf("%s/config.yaml", constants.GardenShootTrustConfiguratorConfigPath),
						},
						...
						VolumeMounts: []corev1.VolumeMount{
							{
								...
								ReadOnly:  true,
								MountPath: constants.GardenShootTrustConfiguratorWebhookTLSCertDir,
							},
							{
								Name:      constants.GardenShootTrustConfiguratorConfigName,
								ReadOnly:  true,
								MountPath: constants.GardenShootTrustConfiguratorConfigPath,
							},
						},
					}},
	...
	metricsPortNetPol := networkingv1.NetworkPolicyPort{
		Port:     ptr.To(intstr.FromInt32(metricsPort)),
		Protocol: ptr.To(corev1.ProtocolTCP),
	}
	if err := gutil.InjectNetworkPolicyAnnotationsForGardenScrapeTargets(service, metricsPortNetPol); err != nil {
		return nil, err
	}

	webhookPortNetPol := networkingv1.NetworkPolicyPort{
		Port:     ptr.To(intstr.FromInt32(webhookPort)),
		Protocol: ptr.To(corev1.ProtocolTCP),
	}
	if err := gutil.InjectNetworkPolicyAnnotationsForWebhookTargets(service, webhookPortNetPol); err != nil {
		return nil, err
	}

	serviceMonitor := &monitoringv1.ServiceMonitor{
		ObjectMeta: monitoringutils.ConfigObjectMeta(constants.GardenShootTrustConfiguratorApplicationName, namespace, "garden"),
...
func getTargetResources(webhookCaBundle []byte, namespace, gardenAccessServiceAccountName string) (map[string][]byte, error) {
	var (
		...
		validatingWebhookURL = fmt.Sprintf("https://%s.%s/webhooks/oidc", constants.GardenShootTrustConfiguratorApplicationName, namespace)
	)
		...
		&rbacv1.Role{
			...
			Rules: []rbacv1.PolicyRule{
				...
				},
				{
					APIGroups:     []string{"coordination.k8s.io"},
					Resources:     []string{"leases"},
					ResourceNames: []string{trustconfigv1alpha1.DefaultLockObjectName},
			...
			Webhooks: []admissionregistration.ValidatingWebhook{{
				...
				ObjectSelector: &metav1.LabelSelector{
					MatchLabels: map[string]string{
						"app.kubernetes.io/managed-by": constants.GardenShootTrustConfiguratorApplicationName,
					},
				},
...
func getLabels() map[string]string {
	return map[string]string{
		"app.kubernetes.io/name": constants.GardenShootTrustConfiguratorApplicationName,
	}
}
